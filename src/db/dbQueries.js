...
var connectioncheck = connection.connect((err)=>{
    if(err){
...
const addRestaurant = (data, callback) => {
    try {
        const sql = `INSERT INTO restaurantlogin_tb(\`r_name\`, \`r_address\`, \`r_email\`, \`r_password\`, \`r_image\`, \`r_city\`,\`r_latitude\`,\`r_longitude\`) VALUES('${data.name}', '${data.address}', '${data.email}', '${data.password}', '${data.image}', '${data.city}','', '')`;
        connection.query(sql, (error, results, fields) => {
            if(error) {
                callback(error)
            }
